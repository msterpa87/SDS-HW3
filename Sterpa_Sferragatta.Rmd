...
We start by computing the correlation matrices for each subject and the pooling this data together taking the average as they are supposed to be independent. Whenever we average the correlation we first transform them to the z-scale so that asymptotically they all have the same variance and hopefully get a more robust average and avoid that one subject who shows very different correlations throws off the pooled estimate.
...
From the graph it's quite clear that the Bonferroni adjustment is necessary to avoid a huge amount of false discovery (edges).
...
A_means = colMeans(atanh(A_flat))
B_means = colMeans(atanh(B_flat))
G_delta = tanh(abs(A_means - B_means))

# Plot of differences at different thresholds
t_range = quantile(G_delta, probs = c(0.85, 0.95), na.rm = T)
...
In order to build bootstrap confidence intervals for the correlation difference we sample multiple times subjects in both groups, compute the average correlation (working in the z-scale) and keep track of the difference in absolute value for each pair of features. Finally we build a normal CI and try to gain some insight from the resulting graph at different threshold values. It looks like the 80th percentile captures a few communities which are "highly connected" suggesting their respective features show a marked difference in correlation between the two groups.

```{r}
b = 300

# point estimate correlation
cor_a = colMeans(atanh(A_flat))
cor_b = colMeans(atanh(B_flat))
...
for(i in 1:b) {
  ...
  D_boot[i,] = abs(colMeans(atanh(A_flat[idx_a,])) - colMeans(atanh(B_flat[idx_b,])))
}

# Normal confidence interval
ci_normal = matrix(NA, nrow=2, ncol=116^2)
se = apply(D_boot, MARGIN = 2, sd)
ci_normal[1,] = tanh(cor_a - qnorm(1-alpha) * se)
ci_normal[2,] = tanh(cor_a + qnorm(1-alpha) * se)

# Adjacency matrices
t = tanh(quantile(diff, probs=c(0.5,.6,.7,.8), na.rm=T))
par(mfrow=c(2,2), mai=c(.1,.1,.3,.1))

for(i in 1:4) {
  M_diff = get_adjancency_matrix(ci_normal[1,], ci_normal[2,], t[i])
  plot_graph(M_diff, title = paste("t = ", round(t[i], 2)), community = T)
}
```
